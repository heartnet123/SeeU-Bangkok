...
describe("planning tools with Mapbox Directions", () => {
	...
	test("plan_itinerary applies silent personalization defaults when constraints are generic", async () => {
		globalThis.fetch = (async (input: RequestInfo | URL) => {
			const url = String(input);
		...
		expect(draft.constraints.budgetLevel).toBe("low");
		expect(draft.constraints.themes).toContain("cafe");
		expect(draft.constraints.maxStops).toBe(3);
	});
});
